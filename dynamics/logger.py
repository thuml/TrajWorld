...
class WandbOutputHandler(KVWriter):
    def __init__(self, dir: str, **config) -> None:
        ...
        task_name, algo_name, exp_name = dir.split("/")[-4:-1]
        wandb_config = yaml.safe_load(open('wandb.yaml', 'r'))
        wandb.init(
            name=exp_name,
            group=task_name + "/" + algo_name,
            dir=dir,
            **wandb_config,
        )
        self.step = 1
    ...
    def add_hyper_params_to_wandb(self, hyper_param: Dict) -> None:
        wandb.config.update(hyper_param)

    def writekvs(self, kvs: Dict) -> None:
        wandb.log({"step": self.step, **kvs})
...
class Logger(object):
    ...
    def _init_ouput_handlers(self, output_config: Dict) -> None:
        ...
        for file_name, fmt in output_config.items():
            try:
                if fmt == "wandb":
                    self._output_handlers.append(HANDLER[fmt](self._wandb_dir))
                else:
    ...
    def log_hyperparameters(self, hyper_param: Dict) -> None:
        ...
        for handler in self._output_handlers:
            ...
            if isinstance(handler, WandbOutputHandler):
                handler.add_hyper_params_to_wandb(hyper_param)
...
def make_log_dirs(
    ...
    if exp_prefix is not None:
        exp_name = f"{exp_prefix}&{exp_name}"
    log_dirs = os.path.join(root_dir or ROOT_DIR, task_name, algo_name, exp_name)
