...
def step_trm_for_pretrain(
    ...
    pred = trm.apply_fn(trm.params, inputs_probs, obs_act_indicator, padding_masks, variate_masks,
                        ...
                        method=TrajWorldTransformer.call_variate_mask)
...
def update_trm(
    ...
    def loss_fn(params: Params) -> jnp.ndarray:
        pred = trm.apply_fn(params, inputs, obs_act_indicator, padding_masks, training=True, variate_key=variate_key, rngs={'dropout': key})
        pred, target = pred[:, :-1, :obs_dim + 1], targets[:, 1:, :obs_dim + 1]
        ...
        if weighted_loss:
            ...
            loss = loss.mean(-1)
        loss = (padding_masks[:, :-1] * loss).sum() / padding_masks[:, :-1].sum()
        return loss, {
            "loss": loss,
        }
...
@partial(jax.jit, static_argnames=('obs_dim', 'n_times_update', 'micro_batch_size', 'weighted_loss', 'input_discrete', 'target_discrete'))
...
def eval_trm(
    ...
    pred_prob = jax.nn.softmax(pred)
    pred_values = transform_from_probs(pred_prob, support)[:, :-1, :obs_dim + 1]
    target_values = inputs[:, 1:, :obs_dim + 1]

    loss = optax.softmax_cross_entropy(pred[:, :-1, :obs_dim + 1], targets_probs[:, 1:, :obs_dim + 1])
    if weighted_loss:
        weight = jnp.maximum(support[:, -1] - support[:, 0], 0.1)[:obs_dim + 1]
        weight = weight / (weight.sum() + 1e-6)
        loss = (weight * loss).sum(-1)
    ...
    mse = (padding_masks[:, -2: -1] * mse.mean(-1)[:, -1:]).sum() / padding_masks[:, -2: -1].sum()
    obs_mse = jnp.square(pred_values[:, :, :obs_dim] - target_values[:, :, :obs_dim])
    obs_mse = (padding_masks[:, -2: -1] * obs_mse.mean(-1)[:, -1:]).sum() / padding_masks[:, -2: -1].sum()
    rew_mse = jnp.square(pred_values[:, :, obs_dim] - target_values[:, :, obs_dim])
    ...
    obs_abs_err = jnp.abs(pred_values[:, :, :obs_dim] - target_values[:, :, :obs_dim])
...
def update_trm_for_pretrain1(
    ...
    def loss_fn(params: Params) -> jnp.ndarray:
        ...
        loss = optax.softmax_cross_entropy(pred, target)
        loss = (loss * obs_select).sum(-1) / obs_select.sum(-1)
        loss = (padding_masks[:, :-1] * loss).sum() / padding_masks[:, :-1].sum()
        return loss, {
            "loss": loss,
        }
...
def eval_trm_for_pretrain(
    ...
    mse = jnp.square(pred_values - target_values)
    mse = (mse * obs_select).sum(-1) / obs_select.sum(-1)
    mse = (padding_masks[:, -2: -1] * mse[:, -1:]).sum() / padding_masks[:, -2: -1].sum()

    return rng, {"loss": loss, "mse": mse}



class TrajWorldDynamics(object):
    def __init__(self, config: MOPOConfig, max_values: np.ndarray, min_values: np.ndarray, knn_data: Optional[np.ndarray] = None):
        self.config = config
        ...
        max_timestep = config.history_length
        rng = jax.random.PRNGKey(config.seed)
        rng, init_key = jax.random.split(rng, 2)
        trm_model = TrajWorldTransformer(
    ...
    def train(
        ...
        if holdout_eps_len:
            ...
            holdout_indices = np.array([i * eps_len + j for i in indices[train_size // eps_len: train_holdout_size // eps_len]
        ...
        if percentage_for_training < 1.0:
            print(f"Using {percentage_for_training} of dataset for training")
            data_size = inputs.shape[0]
            size = int(data_size * percentage_for_training)
    ...
    def train_with_dataloader(
        ...
        def preprocess_batch(batch=False):
        ...
        for batch in train_loader:
            history, obs_act_indicator, history_mask, variate_masks, obs_select, support, sigma = preprocess_batch(batch)
            ...
            if updated_iters % val_interval == 0:
                def validate(val_name, one_val_loader):
                    ...
                    for i, val_batch in enumerate(one_val_loader):
                        history, obs_act_indicator, history_mask, variate_masks, obs_select, support, sigma  = preprocess_batch(val_batch)
    ...
    def preprocess_batch(self, batch=False):
        history, obs_act_indicator, history_mask = batch['history'], batch['obs_act_indicator'], batch['history_mask']
        act_dim = obs_act_indicator[0, 0].sum().item()
        obs_dim = obs_act_indicator.shape[-1] - act_dim - 1
        ...
        padding_variate_dim = max_num_variates - obs_dim - 1 - act_dim
        history = jnp.concatenate([history, jnp.zeros(history.shape[:-1] + (padding_variate_dim,))], axis=-1)
        obs_select = jnp.concat(
            [1.0 - obs_act_indicator, jnp.zeros(obs_act_indicator.shape[:-1] + (padding_variate_dim,), dtype=jnp.int8)],
            axis=-1)
