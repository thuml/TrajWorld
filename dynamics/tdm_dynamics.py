        ...
        for i, val_batch in enumerate(one_val_loader):
            ...
            for j in range(rollout_len):
                for k in range(obs_dim + 1):
                    if k == 0:
                        self.rng, samples =  step_trm_for_pretrain(
                            self.rng, self.trm,
                            history, obs_act_indicator, history_mask, variate_masks, obs_select, support, sigma,
                            input_discrete=self.config.trm_input_discrete,
                            target_discrete=self.config.trm_target_discrete,
                        ...
                        history = jnp.concat([history, padded_pred], axis=1)
                        history_mask = jnp.concat([history_mask, jnp.ones((samples.shape[0], 1))], axis=1)
                    elif k < obs_dim:
                        self.rng, samples = step_trm_for_pretrain(
                            self.rng, self.trm,
                            history, obs_act_indicator, history_mask, variate_masks, obs_select, support, sigma,
                        ...
                        pred = samples[:, :, k - 1:k]
                        history = history.at[:, -1:, k:k + 1].set(pred)
                    else:
                        ...
                        history = history.at[:, -1:, k:k + 1].set(rewards)
                # set action
                history = history.at[:,-1, 9].set(real_history[:, history_len + j, 9])
