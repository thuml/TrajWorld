...
class Logger(object):
    ...
    def _try_sw_log_image(self, key, image, step):
        if self._sw is not None:
            assert image.ndim == 3
            self._sw.image(key, image, step)

    def _try_sw_log_video(self, key, frames, step):
        if self._sw is not None:
            frames = jnp.array(frames)
            frames = jnp.expand_dims(frames, 0)
            self._sw.video(key, frames, step, fps=30)

    def _try_sw_log_histogram(self, key, histogram, step):
        if self._sw is not None:
            self._sw.histogram(key, histogram, step)
    ...
    def log_param(self, key, param, step):
        self.log_histogram(key + '_w', param['params']['weight'], step)
        if 'weight' in param['params'] and param['params']['weight'].grad is not None:
            self.log_histogram(key + '_w_g', param['params']['weight'].grad, step)
        if 'bias' in param['params']:
            self.log_histogram(key + '_b', param['params']['bias'], step)
            if param['params']['bias'].grad is not None:
                self.log_histogram(key + '_b_g', param['params']['bias'].grad, step)
