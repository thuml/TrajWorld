...
from dynamics.tdm_dynamics import TDM_Dynamics, transform_to_onehot, transform_to_probs, transform_from_probs, transform_from_probs_sample
...
Run Example:
python pred/pred_mse_tdm.py --env walker2d-random-v2 --model_path trm_dynamics.pkl
'''

def eval_tsm(
        dynamics: TDM_Dynamics,
        data: HistoryBatch,
        batch_size: int = 1024,
    ) -> None:
...
if __name__ == "__main__":
    ...
    for leval in ['random', 'medium', 'medium-replay', 'medium-expert', 'expert']:
        ...
        """Get architecture"""
        model = TDM_Dynamics(config, max_values, min_values)
