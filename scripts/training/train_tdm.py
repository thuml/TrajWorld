...
from dynamics.tdm_dynamics import TDM_Dynamics, transform_to_probs, \
...
"""
python scripts/training/train_tdm.py env_name=hopper-medium-replay-v2 log_root_dir=log_model_new trm_epoch_steps=5000 dynamics_max_epochs_since_update=300 dynamics_max_epochs=50 seed=183 train_model_only=true exp_name=tdm_ft trm_lr=1e-5 load_pt_dynamics_path="mergeall_pt/model/trm_dynamics990000.pkl" n_blocks=6
python scripts/training/train_tdm.py env_name=hopper-medium-replay-v2 log_root_dir=log_model_new trm_epoch_steps=5000 dynamics_max_epochs_since_update=300 dynamics_max_epochs=50 seed=183 train_model_only=true exp_name=tdmscratch trm_lr=1e-5 n_blocks=6
"""
...
def _rollout_kernel(
    ...
    # rollout
    histories, history_masks = init_histories[:, -trm_lookback_window - 1: -1], init_history_masks[:,
                                                                                -trm_lookback_window - 1: -1]
    masks = None
    if use_kv_cache:
    ...
    for _ in range(rollout_length):
        ...
        def step(
            ...
            if uncertainty_mode == "pred_std":
                ...
                pred_std = (jnp.std(samples, axis=0) / (
                            dynamics_max_values[:obs_dim + 1] - dynamics_min_values[:obs_dim + 1])).sum(-1)
                samples = samples[0]
        ...
        if use_kv_cache:
            ...
            for i in range(0, len(observations), max_batch_size):
                rng, _next_observations, _rewards, _terminals, _info = step(
                    rng, histories[i:i + max_batch_size, -trm_lookback_window:],
                    history_masks[i:i + max_batch_size, -trm_lookback_window:])
                next_observations.append(_next_observations)
                rewards.append(_rewards)
                terminals.append(_terminals)
                info.append(_info)
        ...
        if use_diffuser:
            ...
            for j in range(diffuser_iters):
                next_observations, info["raw_reward"] = denoise(observations, actions, next_observations,
                                                                info["raw_reward"])
...
class MOPOTrainer(object):
    ...
    def __init__(self,
        ...
        self.model = TDM_Dynamics(config, max_values, min_values)
    ...
    def jitted_rollout(
        ...
        if self.config.use_med_diffuser:
            extra_args = [True, self.config.denoise_iters,
                          self.diffuser.dynamics.params, self.diffuser.dynamics.apply_fn, self.diffuser.scaler.mu,
                          self.diffuser.scaler.std]
        else:
            extra_args = []
    ...
    def replay_rollouts(self, history):
        ...
        for i in range(len(history)):
            ...
            # next_state, next_reward, done, _ = env.step(action + np.random.randn(self.act_dim) * 1.0)
        ...
        imageio.mimsave(os.path.join(log_dir, f'rollouts{self.log_epoch}.gif'), [np.concatenate([
            x, y, np.abs((np.float32(x) - np.float32(y)) * 5.).clip(0, 255).astype(np.uint8)], 1) for x, y in
            zip(real_frames, fake_frames)], loop=0)
    ...
    def rollout_diagnostic(
        ...
        histories, history_masks = init_histories[:, -
                                                     self._trm_lookback_window:], init_history_masks[:,
                                                                                  -self._trm_lookback_window:]
        ...
        for noise_std in noise_stds:
            ...
            for i in range(0, len(observations), max_batch_size):
                rng, _next_observations, _rewards, _terminals, _info = self.model.step(
                    rng, histories[i:i + max_batch_size, -self._trm_lookback_window:],
                    history_masks[i:i + max_batch_size, -self._trm_lookback_window:])
                next_observations.append(_next_observations)
                rewards.append(_rewards)
                terminals.append(_terminals)
                info.append(_info)
    ...
    def rollout(
        ...
        # rollout
        histories, history_masks = init_histories[:, -
                                                     self._trm_lookback_window - 1:-1], init_history_masks[:,
                                                                                        -self._trm_lookback_window - 1:-1]
        masks = None
        if use_kv_cache:
        ...
        for _ in range(rollout_length):
            ...
            rng, ref_next_observations, ref_rewards, ref_terminals, ref_info = self.ref_model.step(rng, observations,
                                                                                                   actions)
...
if __name__ == "__main__":
    ...
    log_dirs = make_log_dirs(config.env_name, config.algo, config.seed, vars(config),
                             record_params=["penalty_coef", "rollout_length", "bc_weight", "target_q_type"],
                             root_dir=config.log_root_dir, exp_prefix=config.exp_name)
